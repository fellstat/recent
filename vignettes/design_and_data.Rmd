...
For the LAg-Avidity assay, we recommend the use of the q(t) curve from Fellows et. al. [1], which is based on data from [2].  This curve is provided by default in the `rita` R package. For reference FRR, the only study we are aware of that was conducted in a non-elite controller, untreated, non-AIDS population was [2], who reported a reference false recency rate of 0.55%.
...
The `rita` package can be used for the analysis of both RITA3 and RITA2 algorithms. Additionally, a Shiny web application is available
