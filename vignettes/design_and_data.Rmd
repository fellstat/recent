...
Obtaining precise incidence estimates typically requires significant sample sizes. We recommend using this [TODO: Add Link] tool for determining the appropriate sample size.
...
3. **Anti-Retroviral Therapy (ART) biomarkers:** A binary variable distinguishing those currently on ART from those that are not [TODO: Discuss which tests to use]. This should be collected for all HIV+ individuals.
4. **Viral Load:** The viral load in copies per milliliter [TODO: ??].
...
6. **Date of Last HIV Test:** The day, month and year of each individual’s last HIV test should be collected.
7. **Date of Data Collection:** The day, month and year each subject was interviewed should be collected. This, combined with time of last HIV test is used to calculate the time since last HIV test.
...
All of the following must be true to be classified as recent (Treatment RITA):
1. HIV Positive
2. Viral Load >1000 c/ml
3. No ART Biomarkers
4. LAg-Avidity ODn > 1.5

All of the following must be true to be classified as recent (Diagnosis RITA):
1. HIV Positive
2. Viral Load > 1000 c/ml
3. No ART Biomarkers
4. No Previous Diagnosis
5. LAg-Avidity ODn > 1.5

We call a subject RITA recent if all conditions of the RITA are met. We call an individual screened in if all of the non-LAg-Avidity conditions are met (i.e. they are untreated for the Treatment RITA or undiagnosed for the Diagnosis RITA).

Some older studies have used only viral load with no ART biomarkers to screen out treated individuals. We do not recommend this for studies going forward.
...
2 years is often used as a parameter value for tau. Reference FRR and q(t) are usually determined from a reference study in a population with known times since seroconversion. In order to match the screened in study population, these studies should have been performed in non-elite controller, untreated and populations. Additionally, it is reasonable in many populations to assume that individuals who progress to AIDS will be diagnosed due to the presentation of symptoms. Under this assumption the reference population should also be non-AIDS progressed.

For the LAg-Avidity assay, we recommend the use of the q(t) curve from Fellows et. al. (2022), which is based on data from [TODO add citation]. For reference FRR, the only study we are aware of that was conducted in a non-elite controller, untreated, non-AIDS population was [TODO: cite], who reported a reference false recency rate of 0.55%.
